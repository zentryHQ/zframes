...
//
// A demo-data board (no provider installed — the synthetic fleet answers every
// capability with seeded numbers) says so IN the digest. The header badge is
// the only other disclosure and the agent can't see it, so without a marker
// here zAI analyses fabricated prices as real and the answer the user reads
// never mentions it. A market dashboard must never pass generated numbers off
// as live, and that rule has to reach the prompt, not just the chrome.
...
/**
 * The demo-data disclosure, carried at the TOP of the digest rather than only
 * on the readings heading: the digest is truncated from the end, so a marker
 * anywhere else is the first thing a big board loses.
 */
export const SYNTHETIC_DIGEST_NOTE =
  "IMPORTANT — this dashboard has NO live data provider installed. Every reading " +
  "below is SIMULATED demo data generated by zframes, not a real market value. " +
  "Say so in your answer whenever you use one of these numbers.";

/**
 * Assemble the two sections into the plain-text digest. Pure and exported so
 * the disclosure wording is testable without a provider fleet. Returns null
 * when there is nothing to describe — a lone marker line is not grounding.
 */
export function assembleDigest(
  ...
  if (structure.length === 0 && readings.length === 0) return null;
  const sections: string[] = [];
  if (synthetic) sections.push(SYNTHETIC_DIGEST_NOTE);
  if (structure.length)
 ...
 *
 * `synthetic` is the host's "every provider on this board is the demo one" flag
 * (the same one that badges the header); it marks the digest as simulated.
 */
export function useScreenSnapshot(
  ...
  return useCallback(async () => {
    try {
      ...
      return assembleDigest(structure, readings, synthetic);
    } catch {
      return null; // grounding is best-effort; never block the question
    }
  }, [providers, spec, registry, synthetic]);
}
