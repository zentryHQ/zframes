...
const CONRNER_BORDER_RADIUS = '4px';
    ...
    useEffect(() => {
        ...
        if (container) {
            ...
            for (let i = 0; i < childrens.length; i++) {
                ...
                const topLeftChildPosition =
                    topLeftChild.getBoundingClientRect();
                const bottomLeftChildPosition =
                    bottomLeftChild.getBoundingClientRect();
                const topRightChildPosition =
                    topRightChild.getBoundingClientRect();
                const bottomRightChildPosition =
                    bottomRightChild.getBoundingClientRect();
                if (
                    x <= topLeftChildPosition.x &&
                    y <= topLeftChildPosition.y
                ) {
                    topLeftChild = child;
                }
                if (
                    x <= bottomLeftChildPosition.x &&
                    y >= bottomLeftChildPosition.y
                ) {
                    bottomLeftChild = child;
                }
                if (
                    x >= topRightChildPosition.x &&
                    y <= topRightChildPosition.y
                ) {
                    topRightChild = child;
                }
                if (
            ...
            if (topLeftChildRef.current) {
                topLeftChildRef.current.style.borderTopLeftRadius =
                    BORDER_RADIUS;
            }
            topLeftChildRef.current = topLeftChild;
            topLeftChildRef.current.style.borderTopLeftRadius =
                CONRNER_BORDER_RADIUS;
