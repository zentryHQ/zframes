...
interface InteractionHandlers {
  ...
  onMouseLeave: () => void;
  /** Disconnect the ResizeObserver this created; call on effect cleanup. */
  destroy: () => void;
}
...
export const createInteractions = ({
  ...
  const updateHoverDots = (dataPoint: CombinedDataPoint, xPos: number) => {
    ...
    // Drawn hover knob — a soft glow ring plus a solid core — so the chart
    // layer ships no external bitmap asset (keeps the OSS build dependency-free).
    newGroups
      .append("circle")
  ...
  const calculateDesktopTooltipPosition = (mouseX: number, mouseY: number) => {
    if (
      tooltipRef.current &&
      cachedTooltipHeight === 0 &&
      cachedTooltipWidth === 0
    ) {
      cachedTooltipHeight = tooltipRef.current.getBoundingClientRect().height;
      cachedTooltipWidth = tooltipRef.current.getBoundingClientRect().width;
    }
  ...
  const calculateMobileTooltipPosition = (mouseX: number) => {
    ...
    if (mouseX + tooltipWidth / 2 > cachedContainerWidth - CHART_MARGIN.left) {
  ...
  const updateDesktopTooltipContent = (
    ...
    const dateElement = tooltipRef.current.querySelector("[data-tooltip-date]");
  ...
  const handleMouseMove = (event: MouseEvent | TouchEvent) => {
    ...
    const formattedDate = d3.timeFormat("%b %d, %Y - %H:%M")(dataPoint.date);
    ...
    if (containerWidth > CHART_BREAKPOINTS.sm) {
      ...
      updateTooltipsStyles(desktopTooltipPosition.x, desktopTooltipPosition.y);

      updateDesktopTooltipContent(formattedDate, dataPoint.values);
    } else {
      const mobileTooltipPosition = calculateMobileTooltipPosition(mouseX);

      updateTooltipsStyles(mobileTooltipPosition.x, mobileTooltipPosition.y);
