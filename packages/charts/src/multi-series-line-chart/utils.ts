...
export const findClosestDataPoint = (
  ...
  return date.getTime() - d0.date.getTime() > d1.date.getTime() - date.getTime()
    ? d1
    : d0;
};
...
export const sortSeriesByLastValue = (
  ...
  return [...series].sort((a, b) => {
    const aLastValue = a.data.length > 0 ? a.data[a.data.length - 1].value : 0;
    const bLastValue = b.data.length > 0 ? b.data[b.data.length - 1].value : 0;
