...
| Name→value row | `MetricRow` (`./metric-row`) — both text lines carry a native `title`, so a clipped label stays readable | a new macro-list row |
| A figure that has NO reading this period | the `absent` flag on `CardHeader.Value` / `Stat.Value` / `MetricRow`, and `ABSENT` (`./format`) for the em dash itself — greys the placeholder so it can't read as data | a bare `"—"` in the figure's own ink, or a `text-disabled` class stacked on a size that already sets ink |
| Gainers/losers split for a two-column movers card | `splitMovers(rows, count)` (`./mover-row`) — clamped at the midpoint, so no row lands in both columns | `rows.slice(0, n)` against `rows.slice(-n).reverse()` |
...
| Reduced motion driven from JS (fades, shuffles, auto-advance) | `useReducedMotion()` (`./use-reduced-motion`) — live `matchMedia` subscription, flips mid-session | sampling `prefersReducedMotion()` once at mount, or a per-frame `matchMedia` |
| "Is anyone looking at this card" as RENDER state (defer an embed, pause a game) | `useFrameActivity()` (`./use-frame-active`) — viewport ∩ page-visible | a hand-rolled `IntersectionObserver`, or `useVisibilityRef` when you need a re-render |
| Gauge bounds + absent reading | `GaugeReading` / `GaugeOffScale` / `gaugeRingValue` (`./gauge-scale`) — marks a reading past its bound as text, renders non-finite as the em-dash placeholder | clamping the ring and printing the raw figure beside it |
...
- **An absent figure is greyed, and a non-finite one never prints.** Every formatter in `./format` returns `ABSENT` (one em dash) for `NaN`/`Infinity`, and `changeColor` returns `currentColor` rather than loss-red — `NaN >= 0` is false, so the plain comparison painted every failed computation in a confident red. The placeholder itself must not sit in the figure's ink: pass `absent` to the shared head/stat/row rather than writing a bare `"—"`, because a dash set in `metric-xl text-strong` reads as a value. Ink is split OUT of the size class in both `card-header.tsx` and `stat.tsx` for this reason: two ink utilities on one element let the stylesheet's order decide the winner, not the caller.
