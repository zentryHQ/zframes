...
| Time-series line chart | **`TimeSeriesChart`** (`./series-chart`) — `MultiSeriesLineChart` plus the board's event markers | importing `MultiSeriesLineChart` from `@zframes/charts` (silently drops the card's event flags) |
...
- **Money on market data goes through `useMoney()`, not `formatPrice`.** A card can be denominated in any of 19 currencies (dashboard `currency.code`, or a per-card override), and `tests/currency-coverage.test.ts` **fails the build** if a frame outside its `USD_ONLY` list touches `formatPrice`/`formatCompactUsd`. The plain `$` helpers are only for US-macro series, SEC figures as reported, and user-typed numbers. The hook is only callable from a component: for a nested React component (treemap `Leaf`, heatmap `Cell`) call `useMoney()` inside it — before any early return; for a D3 render callback that is NOT a component (`formatTitle`, `formatValue`), call the hook in the frame and let the closure capture `money` (see `market-bubbles`), or pass it down as a prop (see `order-book-depth`).
- **A time-series chart renders through `TimeSeriesChart`, not the raw chart.** The dashboard's `events` (dated markers every history chart draws) reach a card only through that wrapper, and `tests/chart-events-coverage.test.ts` **fails the build** on a direct `MultiSeriesLineChart` import — a chart that quietly opted out looks identical to a week with no news, so nobody would catch it in review. Pass `events` explicitly only to override the board's list for one chart.
