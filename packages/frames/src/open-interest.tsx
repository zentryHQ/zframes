import { parseMarketData } from "@zframes/charts";
    ...
    <div className="flex h-full flex-col gap-1.5 overflow-y-auto">
      {rows.map((entry) => (
        <div
          ...
          title={`${entry.symbol} · $${parseMarketData(entry.openInterestUsd)} open interest`}
        >
          ...
          <div className="relative h-4 w-full overflow-hidden rounded-sm bg-white/[0.06]">
            <div
              ...
              style={{
                ...
                background: "hsl(var(--zf-accent-hue, 242) 85% 65% / 0.5)",
              }}
            />
          ...
          <span className="caption text-soft text-right tabular-nums">
            ${parseMarketData(entry.openInterestUsd)}
          </span>
        </div>
      ))}
    </div>
