...
describe("buildPrompt", () => {
  ...
  it("calls a live digest live and adds no demo-data rule", async () => {
    const file = writeSpec({ title: "X", frames: [] });
    const prompt = await buildPrompt(
      file,
      "what's moving?",
    ...
    expect(prompt).toContain("live values captured from the screen");
    expect(prompt).not.toContain("SIMULATED");
  });

  it("marks a synthetic digest as simulated and tells the answer to say so", async () => {
    const file = writeSpec({ title: "Demo Board", frames: [] });
    const prompt = await buildPrompt(
      file,
      "what's moving in BTC?",
      "Prices: BTC $60,000.00 (+1.2%)",
      undefined,
      undefined,
      true,
    );
    // The grounding header itself stops claiming the values are live.
    expect(prompt).toContain("SIMULATED demo values");
    expect(prompt).not.toContain("live values captured from the screen");
    // And the answer is instructed to disclose it — the badge in the header is
    // not something the reader of the answer can see.
    expect(prompt).toContain(
      "SIMULATED demo data generated by zframes, not real market values",
    );
    expect(prompt).toContain("zframes providers add keyless");
    // The readings still reach the agent; only their label changed.
    expect(prompt).toContain("Prices: BTC $60,000.00 (+1.2%)");
  });

  it("skips the demo-data rule when the flag arrives without a digest", async () => {
    const file = writeSpec({
      title: "X",
      frames: [{ config: { symbol: "BTC" } }],
    });
    const prompt = await buildPrompt(
      file,
      "hi",
    ...
    // Nothing was captured, so there are no readings to disclose — a rule about
    // numbers that aren't in the prompt would just be noise.
    expect(prompt).not.toContain("SIMULATED");
    expect(prompt).toContain("The symbols on screen right now are: BTC.");
  });
