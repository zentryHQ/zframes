 ...
 *
 * `synthetic` says the digest's readings are the demo provider's generated
 * numbers rather than live market data, which the answer has to disclose.
 */
export async function buildPrompt(
  ...
  const grounding = trimmed
    ? `Here is what the user is looking at on their dashboard "${title}" right now ` +
      `(${
        synthetic
          ? "SIMULATED demo values — this installation has no live data provider"
  ...
  // Demo-data disclosure. The digest carries its own marker line (the runtime's
  // screen-context builds it), but the *answer* is what the user reads, and a
  // market dashboard must never pass generated numbers off as live — so the
  // instruction to say so is a rule of its own, not a hint buried in grounding.
  const syntheticRule =
    synthetic && trimmed
      ? `The readings above are SIMULATED demo data generated by zframes, not real ` +
        `market values: this installation has no data provider installed. State that ` +
        `plainly in your answer whenever you refer to one of them, do not present any ` +
        `of them as a real price or a real market move, and mention that running ` +
        `\`zframes providers add keyless\` connects free live data.\n\n`
      : "";
