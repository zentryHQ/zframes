...
/**
 * Interactive, in-browser dashboard editor — the same drag/resize/add/delete
 * "customise mode" as the nexus workspace, on a GridStack 12-column grid.
 *
 * Unlike nexus (which persists layouts to localStorage), edits round-trip the
 * human-editable dashboard.json: `onSave` receives the full updated spec, and
 * the host writes it back to disk (dev) or downloads it. The artifact the
 * agent generates and the one a human drags around stay the same file.
 *
 * GridStack owns the DOM of each grid item, so — mirroring nexus — every frame
 * renders into its own React root mounted in the item's content node. The
 * roots reuse the host's shared provider instances via FramesProvider (no
...
export function DashboardEditor({
  ...
  const defaultConfig = useCallback((def?: AnyFrameDefinition): Record<string, unknown> => {
    if (!def) return {}
  ...
  const uniqueId = useCallback((frame: string): string => {
    ...
    while (instancesRef.current.has(id)) id = `${frame}-${++counterRef.current}`
    return id
  }, [])
  ...
  const renderInstance = useCallback((id: string) => {
    ...
    root.render(
      <FramesProvider providers={providersRef.current}>
        <FrameContent instance={instance} registry={registryRef.current} className="zf-fill" />
      </FramesProvider>,
    )
  }, [])
  ...
  const buildItemEl = useCallback((instance: FrameInstance): GridItemHTMLElement => {
  ...
  const restore = useCallback(
    (frames: FrameInstance[]) => {
      ...
      grid.removeAll(true)
      grid.el.querySelectorAll('.grid-stack-item').forEach((node) => node.remove())
  ...
  useEffect(() => {
    ...
    grid.getGridItems().forEach((el) => {
      el.classList.toggle('zf-selected', el.getAttribute('gs-id') === selectedId)
    })
  }, [selectedId, count])
    ...
    <>
      ...
      <div className={editing ? 'zf-editor zf-customise' : 'zf-editor'}>
        <div className="zf-editor-bar">
          {editing && <span className="zf-editor-hint">drag to move · drag a corner to resize · hover to delete</span>}
          <div className="zf-editor-bar-spacer" />
          {!editing ? (
            <>
              <button type="button" className="zf-btn" onClick={() => download(collectSpec())}>
                Export JSON
              </button>
              <button type="button" className="zf-btn zf-btn--primary" onClick={startCustomise}>
                Customise
              </button>
            </>
          ) : (
            <>
              <button type="button" className="zf-btn zf-btn--danger" onClick={clearAll}>
                Clear
              </button>
              <button type="button" className="zf-btn zf-btn--ghost" onClick={cancel}>
                Cancel
              </button>
              <button type="button" className="zf-btn zf-btn--primary" onClick={save}>
                Save
              </button>
            </>
          )}
              ...
              <section>
                ...
                <div className="zf-palette">
                  {paletteFrames.map((def) => (
                    <div key={def.name} className="zf-newwidget" data-frame={def.name}>
                      <div className="zf-newwidget-name">{def.name.replace(/-/g, ' ')}</div>
...
function ConfigPanel({
  ...
  const instance = selectedId ? instancesRef.current.get(selectedId) : undefined
  ...
  if (!instance) {
    return (
      <section>
        ...
        <p className="zf-rail-empty">Select a frame on the grid to edit its title and settings.</p>
      </section>
    )
  }
  ...
  return (
    <section>
      <h3 className="zf-rail-title">Configure · {instance.frame.replace(/-/g, ' ')}</h3>
